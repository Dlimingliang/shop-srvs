...
import (
	...
	"github.com/Dlimingliang/shop-srvs/goods-srv/handler"
	"github.com/Dlimingliang/shop-srvs/goods-srv/initialize"
...
func main() {
	...
	if err != nil {
		zap.S().Panic("goods-srv注册consul失败: " + err.Error())
	}
	go func() {
	...
	go func() {
		...
		if err != nil {
			zap.S().Error("goods-srv注销失败")
		}
		zap.S().Info("goods-srv注销成功")
		close(processed)
	}()
	zap.S().Infof("goods-srv启动成功,%s:%d", *ip, *port)
	<-processed
}
