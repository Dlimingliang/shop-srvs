...
type User struct {
	...
	Gender   bool       `gorm:"column: gender;type:tinyint(1); comment '性别 1:男 2:女'"`
	Birthday *time.Time `gorm:"column: birthday;type:datetime(11); comment '生日'"`
	Role     int        `gorm:"column: role;type:int(2);not null;default:2 comment '角色 1:管理员 2:普通用户'"`
}
